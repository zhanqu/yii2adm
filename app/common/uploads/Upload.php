...
export_ticket
        ...
        if ( ! is_dir($getUploadHeadFile = $this->receiver->getUploadHeadFile()) ) {
            @mkdir($getUploadHeadFile, 0755,true);
        }
    ...
    public function uploading()
    {
        ...
        $this->receiver->uploadHeadFile = $this->receiver->getUploadHeadFilePath();
        ...
        if ( ! ($this->receiver->chunkTotalCount && $this->receiver->chunkIndex && $this->receiver->uploadExt && $this->receiver->uploadBaseName && $subDir) ) {
            return Responser::reportError('缺少必要的文件块参数', true, $this->receiver->uploadHeadFile, $this->receiver->uploadPartialFile);
        }
        # 防止被人为跳过验证过程直接调用保存方法，从而上传恶意文件
        if ( ! is_file($this->receiver->uploadPartialFile) ) {
            return Responser::reportError('此文件不被允许上传', true, $this->receiver->uploadHeadFile, $this->receiver->uploadPartialFile);
        }

       /* if ( $this->receiver->file->getError() > 0 ) {
            return Responser::reportError($this->receiver->file->getErrorMessage(), true, $this->receiver->uploadHeadFile, $this->receiver->uploadPartialFile);
        }*/

      /*  if ( ! $this->receiver->file->isValid() ) {
            return Responser::reportError('文件必须通过HTTP POST上传', true, $this->receiver->uploadHeadFile, $this->receiver->uploadPartialFile);
        }*/
        # 头文件指针验证，防止断线造成的重复传输某个文件块
        if ( @file_get_contents($this->receiver->uploadHeadFile) != $this->receiver->chunkIndex - 1 ) {
            return Responser::returnResult($result);
        }
        # 写入数据到预创建的文件
        if ( $error = $this->receiver->writeFile() ) {
            return Responser::reportError($error, true, $this->receiver->uploadHeadFile, $this->receiver->uploadPartialFile);
        }
        # 判断文件传输完成
        if ( $this->receiver->chunkIndex === $this->receiver->chunkTotalCount ) {
            @unlink($this->receiver->uploadHeadFile);
            # 触发上传完成前事件
            ...
            if ( ! ($result['savedPath'] = $this->receiver->renameTempFile()) ) {
                return Responser::reportError('重命名文件失败', true, $this->receiver->uploadHeadFile, $this->receiver->uploadPartialFile);
            }
